...
const MeetingDetails = () => {
  ...
  if (loading) {
    return (
      <div className="flex min-h-screen bg-slate-50">
        <Sidebar />
        <main className="flex-1 ml-72 p-10 text-slate-600 font-sans flex items-center justify-center">
  ...
  if (!meeting) {
    return (
      <div className="flex min-h-screen bg-slate-50">
        <Sidebar />
        <main className="flex-1 ml-72 p-10 text-slate-600 font-sans">
          <div className="text-center py-20">
            <p className="text-gray-500 font-bold">Meeting not found</p>
            <button onClick={() => navigate('/module3-meetings')} className="mt-4 text-indigo-600 hover:underline font-bold">
              Back to meetings
            </button>
          </div>
        </main>
  ...
  return (
    <div className="bg-slate-50 min-h-screen">
      ...
      <main className="flex-1 ml-72 p-10 text-slate-600 font-sans" style={{minHeight: '100vh'}}>
        {/* Header */}
        ...
        {/* Success Message */}
        {successMessage && (
          <div className="mb-8 bg-indigo-600/10 border border-[#00d09c]/50 text-indigo-600 p-4 rounded-2xl flex items-center gap-3">
            <CheckCircle size={20} />
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="bg-white border border-slate-200 rounded-[2rem] p-8">
              ...
              <div className="space-y-4">
                ...
                {meeting.location && (
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-slate-900 font-bold">{meeting.location}</p>
                    </div>
                  </div>
                )}
                ...
                {meeting.meetingLink && (
                  <div className="flex items-start gap-3">
                    <LinkIcon size={20} className="text-indigo-600 flex-shrink-0 mt-0.5" />
                    <div>
                      <p className="text-gray-500 text-sm">Meeting Link</p>
                      <a href={meeting.meetingLink} target="_blank" rel="noopener noreferrer" className="text-indigo-600 font-bold hover:underline break-all">
                        {meeting.meetingLink}
                      </a>
                    </div>
                  </div>
            ...
            <div className="bg-white border border-slate-200 rounded-[2rem] p-8">
                  ...
                  return (
                    <div
                      ...
                      className={`p-4 rounded-xl border-2 transition-all cursor-pointer flex items-center justify-between ${
                        isSelected
                          ? 'bg-indigo-600/20 border-[#00d09c] text-indigo-600'
                          : 'bg-slate-100 border-slate-200 hover:border-indigo-500/50'
                    ...
                    >
                      ...
                      <span className="bg-indigo-600/10 text-indigo-600 text-sm font-bold px-3 py-1 rounded-lg border border-[#00d09c]/20">
              ...
              {!isFinalized && (
                <button
                  ...
                  className="w-full mt-6 bg-indigo-600 hover:bg-[#00e6ae] text-gray-900 py-3 rounded-xl font-black transition-all flex items-center justify-center gap-2 disabled:opacity-50"
                >
                  <CheckCircle size={18} />
          ...
          <div className="space-y-6">
            ...
            {summary.length > 0 && (
              <div className="bg-white border border-slate-200 rounded-[2rem] p-6">
                ...
                {summary && summary[0] && (
                  <div className="bg-indigo-600/10 border border-[#00d09c]/20 rounded-xl p-4 text-center">
                    <p className="text-slate-900 font-bold text-sm mb-2">{formatDateTime(summary[0].date)}</p>
                    <p className="text-indigo-600 font-black text-2xl">{summary[0].votes}</p>
            ...
            <div className="bg-white border border-slate-200 rounded-[2rem] p-6 space-y-3">
              ...
              {!isFinalized && (
                <button
                  ...
                  className="w-full bg-green-600 hover:bg-green-700 text-slate-900 py-2.5 rounded-xl font-bold transition-all text-sm"
                >
              ...
              <button
                onClick={() => navigate('/module3-meetings')}
                className="w-full bg-gray-800/50 hover:bg-gray-700 text-slate-600 py-2.5 rounded-xl font-bold transition-all text-sm"
              >
                Back to Meetings
              </button>
            </div>
            ...
            <div className="bg-white border border-slate-200 rounded-[2rem] p-6">
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  <span className="text-slate-500 text-sm">Total Proposed Dates</span>
                  <span className="text-slate-900 font-bold">{meeting.proposedDates?.length || 0}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-slate-500 text-sm">Responses Received</span>
                  <span className="text-indigo-600 font-bold">{summary.length > 0 ? summary[0].votes : 0}</span>
                </div>
              </div>
            </div>
          </div>
